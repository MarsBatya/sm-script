...
_sm_completion() {
    ...
    local commands="add start stop logs ls list remove rm workdir dive cd update info install-helpers uninstall help"
    ...
    if [[ $cword -eq 2 ]]; then
        case "$prev" in
            start|stop|logs|remove|rm|workdir|dive|cd|info)
